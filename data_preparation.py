...
def add_accurate_traffic_density(df_trains):
    ...
    # 💡 Varför detta är säkert (inte bara "råkar funka"): groupby(sort=False)
    # besöker grupper i den ordning de först påträffas, och bevarar alltid
    # radordningen INOM varje grupp. Eftersom df redan är sorterat på
    # station_signature -> scheduled_utc innan groupby körs, matchar
    # rolling_counts radordning exakt df:s ordning efter .reset_index(drop=True).
    # Det är dokumenterat pandas-beteende, inte tur - men eftersom hela
    # kopplingen tillbaka till df sker POSITIONELLT (ingen explicit merge på
    # nyckel), lägger vi till en hård kontroll nedan så att koden kraschar
    # tydligt istället för att tyst feljustera värden om något ändras i
    # framtiden (t.ex. om någon lägger till sort=True ovan).
    rolling_counts = (
    ...
    if len(rolling_counts) != len(df):
        raise RuntimeError(
            f"add_accurate_traffic_density: radantal matchar inte "
            f"({len(rolling_counts)} vs {len(df)}) - avbryter för att "
            f"undvika att felaktiga traffic_density-värden smyger sig in."
        )
