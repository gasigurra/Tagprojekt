...
# Om ingen SMHI-station finns närmare än detta (km) för en given parameter,
# varnar vi - stationen mappas ändå mot den närmaste tillgängliga, men det
# är värt att veta att datan kan vara opålitlig för just den kombinationen.
...
def fetch_smhi_stations(param_id):
    ...
    try:
        ...
        print(f"  ⚠️ Nätverksfel vid hämtning av SMHI-stationer (param {param_id}): {e}")
        return []
...
def fetch_trafikverket_stations():
    ...
    try:
        ...
        print(f"  ⚠️ Nätverksfel vid hämtning av Trafikverkets stationer: {e}")
        return stations

    if response.status_code != 200:
        print(f"  ⚠️ HTTP-fel {response.status_code} vid hämtning av Trafikverkets stationer.")
        print(f"  Svar: {response.text[:300]}")
        return stations
    ...
    try:
        ...
        print("Kunde inte tolka Trafikverkets stationssvar:", e)
    return stations
...
def build_and_save_mapping():
    ...
    # KRITISK VALIDERING: utan tågstationer blir hela kartan tom, och alla
    # senare skript (smhi_fetcher.py, train_api.py) kommer tyst sakna
    # väderdata utan att någon märker det förrän långt senare. Avbryt hellre
    # högljutt här.
    if not trv_stations:
        print("❌ Fick inga tågstationer från Trafikverket. Avbryter uppbyggnaden av kartan.")
        print("   Kontrollera API-nyckel och nätverksanslutning, kör sedan om skriptet.")
        return
    ...
    if all(len(stations) == 0 for stations in smhi_networks.values()):
        print("❌ Fick inga SMHI-stationer alls. Avbryter uppbyggnaden av kartan.")
        return
    ...
    for trv in trv_stations:
        ...
        for param_key, smhi_stations in smhi_networks.items():
            ...
            if closest_id is not None and min_dist > MAX_REASONABLE_DISTANCE_KM:
                far_matches += 1
    ...
    if far_matches > 0:
        print(f"ℹ️ Notera: {far_matches} stations-sensorkombinationer matchades mot en SMHI-station "
              f"längre bort än {MAX_REASONABLE_DISTANCE_KM} km (glesbygd/skärgård etc). "
              f"Väderdatan för dessa kan vara mindre träffsäker.")
